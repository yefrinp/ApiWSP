...
//     };
    ...
    //  let store = makeInMemoryStore({ logger: P({ level: 'fatal' }) });

    // // Attempt to read from the 'baileys_store.json' file
    // try {
    //     store.readFromFile('./baileys_store.json');
    //     // console.log('Data read from file:', store); 
    // } catch (error) {
    ...
    await WA.ev.on('messages.upsert', async (messages) => {
      ...
      let update = messages.messages[0].message
      // console.log(update)
      // console.log("update 1")
      let key = messages.messages[0]
...
// https://bot.whatsbotpro.com/api/send?number=84933313xxx&type=media&message=test+message&media_url=https%3A%2F%2Fi.pravatar.cc&filename=file_test.jpg&instance_id=65903C7575F66&access_token=649bebf664a20              
              
              
// const alllist = await WA.groupFetchAllParticipating()
...
// console.log(alllist)
    ...
    await WA.ev.on('call', async (call) => {
      ...
      if (calling.status === 'accept' && item.status === 1) {
        ...
        // Add an additional check for chatId and from
        if (calling.chatId !== calling.from) {
          ...
          console.log("Skipping 'reject' code because chatId and from are the same.");
        }
      } else {
        ...
        // stores.readFromFile("./contact_store.json");

        // setInterval(() => {
        //       stores.writeToFile("./contact_store.json");
        // }, 10_000);

// await WA.ev.on('messaging-history.set', (messages) => {
...
// const instanceDataPath = `./${instance_id}.json`;
//     fs.writeFile(instanceDataPath, JSON.stringify(messages), (err) => {
//         if (err) {
//             console.error('Error writing instance data:', err);
//         } else {
//             console.log(`Instance data saved to ${instanceDataPath}`);
//         }
//     });
    
// });

//   await WA.ev.emit('messaging-history.set', async (allChats) => {
        
//       WAZIPER.webhook(instance_id, { event: "messaging-history.set", data: allChats });
//     });
    

//     await WA.ev.on('chats.set', async () => {
        
//       WAZIPER.webhook(instance_id, { event: "chats.set", data: store.chats.all() });
//     });
    
//       await WA.ev.on('contacts.set', async () => {
        
//       WAZIPER.webhook(instance_id, { event: "contacts.set", data: Object.values(store.contacts) });
//     });
    
      await WA.ev.on('chats.update', async (chatInfoUpdate) => {
      WAZIPER.webhook(instance_id, { event: "chats.update", data: chatInfoUpdate });
    });
    
     await WA.ev.on('contacts.upsert', async (contacts) => {
        //   console.log('from   console.log(contacts)', contacts);
  ...
  get_groups: async function (instance_id, res) {
    ...
    if (client != undefined && client.groups != undefined) {
      console.log(client.groups[0].participants)
      ...
      items.forEach(async (item) => {
        ...
        await accounts.forEach(async (account, index) => {
          if (!instance_id && index == next_account) {
            ...
            var poll = await Common.db_fetch("sp_whatsapp_template", [
              { id: item.template }
              ...
              if (sessions[instance_id] == undefined) {
                ...
                if (items && items.length > 0) {
                  ...
                  if (dbsend % sid === 0) {
                    ...
                    await WAZIPER.auto_send(instance_id, chat_id1, fixedMobileNumber, 'autoresponder', item, params, async function (result) {
  ...
  chatbot: async function (instance_id, user_type, message) {
    ...
    if (chatStates[chat_id] && chatStates[chat_id].waitingForNextMessage) {
        console.log("Processing response message for chat_id:", chat_id);
        chatStates[chat_id].waitingForNextMessage = false;
        console.log("Answer message:", content); // Console the answer message
        return;
    }
    ...
    items.forEach(async (item, index) => {
      ...
      if (run) {
           ...
           if (item.saveresponse) {
                console.log("Waiting for next message for chat_id:", chat_id);
                chatStates[chat_id] = { waitingForNextMessage: true };
                return;
            }
  ...
  auto_send: async function (instance_id, chat_id, phone_number, type, item, params, callback) {
    ...
    if (user_message.pushName) {
      user_name = user_message.pushName;
    } else if (user_message.verifiedBizName) {
      user_name = user_message.verifiedBizName;
    } else {
      user_name = user_message.pushName;
    }
    ...
    if (item.caption.includes("%waname%")) {

      const replacedCaption = item.caption.replace("%waname%", user_name);


      item.caption = replacedCaption;
      ...
      const replacedCaption = item.caption.replace("%wanumber%", formattedPhoneNumber);
      item.caption = replacedCaption;
    ...
    switch (item.type) {
      ...
      case 2:
        ...
        if (poll) {
          const dataString = poll[0].data;
          const dataObject = JSON.parse(dataString);
          const templateButtons = dataObject.templateButtons;
          let textValue = dataObject.text;
          const displayTextValues = templateButtons.map(button => button.quickReplyButton.displayText);
          ...
          if (user_message.pushName) {
            ...
            user_name = message.verifiedBizName;
          } else {
            user_name = message.pushName;
          }

          if (textValue.includes("%waname%")) {

            const replacedCaption = textValue.replace("%waname%", user_name);


            textValue = replacedCaption;


          } if (textValue.includes("%wanumber%")) {

            const phoneNumberParts = phone_number.split('@');
            const formattedPhoneNumber = phoneNumberParts[0];

            const replacedCaption = textValue.replace("%wanumber%", formattedPhoneNumber);
            textValue = replacedCaption;
      ...
      default:
        ...
        let vcard = 'BEGIN:VCARD\n' // metadata of the contact card
          ...
          + 'TEL;type=CELL;type=VOICE;waid=919983939151:+91 99839 39151\n' // WhatsApp ID + phone number
          + 'END:VCARD'
...
case 'png':
    ...
    default:
      // Default case, return a default mimetype or handle as needed
      return "application/pdf"; // Example default mimetype
  }
}
              var data = {
          ...
          async function processGroups(instance_id) {
            var client = sessions[instance_id];
            if (client && client.groups && client.groups[0] && client.groups[0].participants) {
              console.log(client.groups[0]);
              console.log("client.groups");
              const participantIds = client.groups[0].participants.map(participant => participant.id);
          ...
          if (data.caption && data.caption.includes('@all')) {
            ...
            if (numberMatch) {
              ...
              // Set 'participantIds' with the 'withNumber'
              const participantIds = [withNumber];
              data.mentions = participantIds;
          ...
          if (item.type_vcard === "2") {
            ...
            data = {
              contacts: {
                ...
                contacts: [
                  {
                    vcard: vcard // Store the vCard directly, without nested contacts array
                  }
                ]
              }
            };
